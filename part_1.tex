...
\begin{figure}[h]
	...
	\caption[Classification via regression linéaire]{Classification via régression linéaire. On observe sur la figure a) que lorsque la température est inférieure à 12,5 degrés, le système considère que la valeur de sortie est 0 (hiver). Lorsque la température est au dessus, la valeur de sortie devient 1 (été). Ce cas particulier fonctionne car les exemples sont répartis de manière uniforme. Or, dans le cas de la figure b), on remarque que une répartition non uniforme des données entraîne un dysfonctionnement du classement. En effet, on observe  que le le seuil de différentiation entre les deux classes se situe aux alentours de  }
...
Afin de classer des données à partir d'une régression,  on modifie la forme linéaire de l'hypothèse utilisée lors de la régression linéaire en une sigmoïde. 
soit $g(x)$ la fonction sigmoïde, on a l'hypothèse $h_\theta(x)$ suivante: 
