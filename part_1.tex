...
Intuitivement, lorsque l'on observe la forme de la fonction $-log(h(\theta))$ (figure \ref{fig:fonctions -log(h(x)) et -log(1-h(x))}) qui est vraie quand $y=1$, on remarque que lorsque $h(x)$ tend vers 1, le cout s'approche de 0. Lorsque $h_\theta(x)$ tend vers 0, le coût tend vers l'infini. Autrement dit, plus la valeur de l'hypothèse s'approche de la valeur de sortie réelle (y=1),plus le coût est faible et inversement, plus il s'en éloigne, plus il est élevé.  \\
Intuitivement, lorsque l'on observe la forme de la fonction $-log(1-h(\theta))$ (figure \ref{fig:fonctions -log(h(x)) et -log(1-h(x))}) qui est vraie quand $y=0$, on remarque que lorsque $h(x)$ tend vers 0, le cout s'approche de 0. Lorsque $h_\theta(x)$ tend vers 1, le coût tend vers l'infini. Autrement dit, plus la valeur de l'hypothèse s'approche de la valeur de sortie réelle (y=0), plus le coût est faible et inversement, plus il s'en éloigne, plus il est élevé. \\
...
\subsubsection{La classification multi-classe}
...
\subsection{Choix de la méthode à utiliser}
\label{Le Machine Learning: Choix de la méthode à utiliser}

\subsubsection{Choix du type de modèle}
\label{Le Machine Learning: Choix de la méthode à utiliser: Choix du type de modèle}


\subsubsection{Comparaison des algorithmes de classification}
\label{Le Machine Learning: Choix de la méthode à utiliser: Comparaison des algorithmes de classification}

Au regard de l'analyse des différentes méthodes d'apprentissages automatiques réalisées en partie \ref{Le Machine Learning:Les différents algorithmes d'apprentissage supervisé}, on propose de les comparer et de choisir celui qui répondra le plus efficacement à notre problématique initiale, i.e. d'automatiser l'analyse des erreurs apparues lors du déroulement du Filtering Test.

Le premier constat est que notre problème 