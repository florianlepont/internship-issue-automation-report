...
Dans le cadre d'une régression linéaire, la forme de l'hypothèse est linéaire. Dans le cas de la régression logistique, celle-ci ne l'est plus (forme sigmoïde). La fonction coût n'est donc ici pas convexe (graphe convexe de la régression linéaire \ref{fig:Régression linéaire, calcul des paramètres de l'hypothèse}) et on ne peut déterminer un minimum globale, celle-ci pouvant posséder plusieurs minimums locaux.
