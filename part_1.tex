...
La régression linéaire cherche à expliquer une variable de sortie $Y$ par une fonction affine de $X$.  Cette fonction affine est appelée \emph{hypothèse}. Exprimé autrement, on a un jeu de donnée $X$ auquel correspond un jeu de données $Y^$, on cherche les valeurs $\theta_1$ et $\theta_2$ permettant de "mapp
