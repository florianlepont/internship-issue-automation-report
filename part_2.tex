...
L'architecture haut niveau de la solution que l'on propose est composée de deux couches: une couche "root cause" et une couche "\emph{error name}
...
\subsubsection{Exemple de mise en place  d'une couche \emph{error name}
...
On souhaite dans un premier temps mettre en place l'architecture permettant de détecter la cause (root cause) ayant entrainé la chute du robot lors du Filtering test (\emph{error name}
...
}} de notre apprentissage (c.f. partie \ref{Le Machine Learning: Généralités sur le Machine Learning: Définition et principe général:Lexique}). L'ensemble de ces couches root cause sont placées dans la couche \emph{error name}
...
Une fois nos différentes couches root cause crées, on souhaite utiliser notre système afin de détecter la cause de la chute d'un robot (c.f. figure \ref{fig:utilisation de la couche \emph{error name}

}). Pour cela, on place à l'entrée de notre couche \emph{error name}
...
\begin{figure}[h]
	...
	\caption[Utilisation de la couche \emph{error name}
