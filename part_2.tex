...
Entrainer le modèle revient à créer deux patrons:  
\begin{itemize}
...
Pour cela, on entraîne l'algorithme d'apprentissage automatique à partir des deux formes distinctes. grâce aux exemples à cela revient à apprendre la forme globale et caractéristique d'une feature en cas de présence ou non d'une root cause. Une fois cette forme apprise, on la compare avec la forme que l'on souhaite analyser afin de savoir si la root cause est présente ou non dans  
